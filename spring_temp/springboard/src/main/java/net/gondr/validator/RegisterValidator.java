...
public class RegisterValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "userid", "required", "유저 값은 필수입니다.");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "username", "required", "유저 이름은 필수입니다.");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "password", "required", "패스워드는 필수입니다.");
		
		RegisterDTO dto = (RegisterDTO) target;
		
		if(!dto.getPassword().equals(dto.getPasswordConfirm())) {
			//비밀번호와 비밀번호 확인이 틀렸을 경우
